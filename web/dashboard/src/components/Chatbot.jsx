...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    { role: 'ai', content: 'Hello! I am the AXLE AI Copilot. I can help you orchestrate deployments, debug infrastructure issues, or answer questions about your environment.' }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: '[Connection failed: API unreachable]' }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              border: m.role === 'user' ? '1px solid rgba(0,240,255,0.3)' : '1px solid var(--panel-border)',
              padding: '16px 20px',
              borderRadius: '12px',
              ...
              <div style={{ whiteSpace: 'pre-wrap', lineHeight: '1.6', color: '#e2e8f0' }}>{m.content}</div>
      ...
      <form onSubmit={handleSend} style={{ padding: '1.5rem 2rem', borderTop: '1px solid var(--panel-border)', display: 'flex', gap: '1rem' }}>
        <input
          ...
          placeholder="Ask about deployments, infrastructure, or system config..."
          style={{
            flex: 1, padding: '16px 20px', background: 'rgba(0,0,0,0.5)',
